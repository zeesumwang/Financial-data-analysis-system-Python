...
def calculateMa(data, Daycount):
    sum = 0
    result = list( 0 for x in data)#used to calculate ma. Might be deprecated for future versions计算MA
...
def display(items,startdate,enddate,option,width1, height1): #labels:复权ork线or分笔 option:hfq, qfq or 15, 30, D, etc
#    print(items)
    page = Page()
    for i in items:#generate numbers of graphs according to numbers of queries in treewidget
        ...
        if len(j)==3:
            ...
            if a == x: 
                ...
                d1 = ts.get_k_data(j[1],start=startdata,end=enddata)['date']
                data = ts.get_hist_data(j[1],start=startdata,end=enddata)
                shouyi = (data['close']-data['close'].shift(1))/data['close'].shift(1)
                huice = (data['close']-data['close'].shift(1))/data['close'].shift(1)
                zoom = (data['close']-data['close'].shift(1))/data['close'].shift(1)
                zoom1 = (data['close']-data['close'].shift(1))/data['close'].shift(1)
                index=1
                for index in range(len(data['close'])):
                    zoom1[index] = shouyi[index]+shouyi[index-1]
                    if shouyi[index]>0:
                            
                        huice[index] = shouyi[index] * index*0.25
                        zoom[index] = huice[index]+huice[index-1]
                    else:
                        #        print(index)
                        huice[index] = shouyi[index]-(0.0001*index)*(-1)
                        zoom[index] = huice[index]+huice[index-1]
                ...
                # print(len(a))
                #  print(len(b))
                lineshouyi = Line(j[0] , width=width1 * 10 / 11, height=(height1 * 10 / 11) / len(items))
                lineshouyi.add("基准收益",a,b,line_width=2,is_datazoom_show=True, datazoom_type=["slider"], yaxis_interval = 1)
                lineshouyi.add("策略收益",a,hc,line_width=2,is_datazoom_show=True, datazoom_type=["slider"], yaxis_interval = 1)
                lineshouyi.render()
                overlap.add(lineshouyi)
                page.add(overlap)
                return
            ...
            if j[2]!="Kline":
                ...
                if len(a[0])==4 and a[0][2]=="bar": #for 分笔data
                    overlap = Overlap()
                    form = [e[1] for e in a]
                    bar = Bar(j[0] + "-" + j[2], width=width1 * 10 / 11, height=(height1 * 10 / 11))
...
def generateline(stocknumber,Type,startdate,enddate,interval):
    ...
    if Type ==  "ATP model".decode("utf-8"):

        returnarray = [9,9,9,9,9,9,9,9]
        return returnarray
